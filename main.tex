...
% MJW add "tasks" here, right?  Logic-77 is the environment.  Also, I'm going to just note here and not in all of the cases: you are inconsistent in whether it gets called Logic-77 or Logic 77, and whether or not the word Logic is capitalized.  I think you're more consistent with Logic-9, but I'd have to go through and count to determine if I'm right about that.
...
\todo[inline]{reword measuring task discovery explanation above}
...
\todo[inline]{reword measuring task performance above to explain the meaning of ephemeral more clearly}

\section{Results and Discussion}

\subsection{Task Discovery}
% MJW: I suggest a paragraph here about the fact that you looked {set of things comprising the sub-subsections}.  Jumping immediately into a sub-section within a subsection, without something general first, can be a little jarring.
...
We found
% MJW: This sets up tense weirdness.  "We found" is past tense; essentially everything else here is written as present tense.
%@RCK: fixed
...
\todo[inline, color=magenta]{expand result of quiescent recovery above per Mike's comment}
...
\todo[inline]{add note about how simple also perform more tasks than the complex. this is contrary to our expectation}
...
Interestingly, in the complex environments, the harsh quiescent treatments %that were initially evolved in a harsh changing environment and then change ceased upon introduction of Logic-77 rewards (Harsh Quiescent)
...
\todo[inline]{expand speculation per mike's comment of why harsh quiescent outcompeted everyone in the complex treatment}
...
\todo[inline]{speculate - why does this happen?} 
%We also find that the HGT mutagen
% MJW: Flagging a use of mutagen.
%, which helps with short-term evolvability in cyclic environments, does not appear to have a beneficial effect.
% MJW: I'm not sure what your target for publishing this is, but this is a weird format for most science journal articles.  This paragraph reads essentially like a summary of the results, and therefore the sort of thing you might have in an abstract or a conclusion.  I am not used to seeing summaries of the results in the results section, before those things have been specifically laid out (figures, tables, statistical comparisons).  This might be fine for a dissertation; that's up to your committee.  But I'm flagging it as something that's strange for a journal article so that if it stays this way for the diss. you're aware it probably needs to change for publication.
%@RCK: moved to conclusion, possibly for deletion
\todo{clean this up (above)}
...
\todo[inline, color=magenta]{fill this conclusion in, recapitulating the argument and framing about balancing of fitness effects, see mike's note}
...
\todo[inline]{add section about the HGT bonus depressing task exploration in the long term evolution chapter}

\section{Horizontal Gene Transfer and Long Term Evolvability}
\todo[inline, color=magenta]{fill in this section with the single figure}
