...
\dedication{This thesis is dedicated to Kendall Koning. I would never have gotten this far without her.}
\date{2017}
%
\begin{document}
\frontmatter
...
The intact-HGT instruction is used most (Mdn = 87389.71, CI 95\% [82982.98, 96000.3]), followed by the \texttt{HGT-Shuffle} instruction(Mdn = 52220.3, CI 95\% [45094.92, 57465.52]), and then followed by {HGT-Random}(Mdn = 25442.98, CI 95\% [22099.45, 30490.08]), where no information remains.
...
%@RCK: fixed
This indicates that the information content of the fragment is an important predictor of the use of HGT. 
...
\todo{polish figure - show fitness effect instead of uptake in left violin?}
\begin{figure}[h!]
\begin{center}
...
\todo{fix this table to show the halves of the logic 9 split}
	%=[TABLE - experimental treatments]
	\begin{table}[]
	\centering
	...
	\multirow{3}{*}{ \thead{Treatment} } & \multirow{3}{*}{ \thead{Changing \\ Env. \\ Type} } & \multicolumn{5}{c|}{ \thead{Rewarded Tasks} } \\\cline{3-7}
	& & \multicolumn{2}{c||}{ \thead{Part 1 \\ (0-200,000 Updates)} } & \multicolumn{3}{c|}{ \thead{Part 2 \\ (200,000-400,000 Updates)} } \\\cline{3-7}
	& & Group 1 & Group 2 & Group 1 & Group 2 & \makecell{Logic-77 \\ (all)} \\\hhline{|=|=|=|=|=|=|=|}
	Control & \makecell{None \\ (static)} & \makecell{constant \\ $2^{1-5}$} & \makecell{constant \\ $2^{1-4}$} & \makecell{constant \\ $2^{1-5}$} & \makecell{constant \\ $2^{1-4}$} & \makecell{constant \\ $2^{0.3}$} \\\hline
	Benign & Cyclic & \makecell{benign \\ fluctuating \\ $2^{1-5}$ or 0} & \makecell{benign \\ fluctuating \\ 0 or $2^{1-5}$} & \makecell{constant \\ $2^3$} & \makecell{benign \\ fluctuating \\ $2^{1-5}$ or 0} & \makecell{constant \\ $2^{0.3}$} \\\hline
	\makecell{Benign \\ Quiesce} & Cyclic & \makecell{constant \\ $2^3$} & \makecell{benign \\ fluctuating \\ 0 or $2^5$} & \makecell{constant \\ $2^3$} & \makecell{constant \\ $2^5$} & \makecell{constant \\ $2^{0.3}$} \\\hline
	Harsh & Cyclic & \makecell{constant \\ $2^3$} & \makecell{harsh \\ fluctuating \\ $-2^5$ or $2^5$} & \makecell{constant \\ $2^3$} & \makecell{harsh \\ fluctuating \\ $-2^5$ or $2^5$} & \makecell{constant \\ $2^{0.3}$} \\\hline
	\makecell{Harsh \\ Quiesce} & Cyclic & \makecell{constant \\ $2^3$} & \makecell{harsh \\ fluctuating \\ $-2^5$ or $2^5$} & \makecell{constant \\ $2^3$} & \makecell{constant \\ $2^5$} & \makecell{constant \\ $2^{0.3}$} \\\hline
	%\makecell{Harsh \\ to \\ Benign} & Cyclic & \makecell{constant \\ $2^3$} & \makecell{harsh \\ fluctuating \\ $-2^5$ or $2^5$} & \makecell{constant \\ $2^3$} & \makecell{benign \\ fluctuating \\ 0 or $2^5$} & \makecell{constant \\ $2^{0.3}$} \\\hline
	...
	\begin{flushleft} Four types of cyclic changing environment, plus a static control. Each is split into two phases.
    % MJW: These are fragments.
    The first phase is a complex changing environment similar to those found in Chapter~\ref{chap:evo-hgt}, Table~\ref{hgt-treatments-h}. The second phase introduces an additional set of tasks (Logic-77) that are rewarded at a lower rate.
	\end{flushleft}
...
In contrast, the benign environment, with its comparatively weaker strength of selection for EQU task gain and loss, experiences a comparatively stronger selective pressure to acquire logic-77 tasks, while still benefiting from the increased exploration rate conveyed by the benign changing environment.
	...
	\caption{\textbf{Number of new logic-77 tasks discovered beginning in part 2}. [TODO DESCRIPTION AND STATS]%
	}
...
populations seem to outperform the control, this effect is only statistically significant in the complex environment (Wilcoxon Rank Sum Test: Z = 2.42 and 2.37 respectively, p $<$ 0.05). In the simple environment, the differences are not statistically significant (Kruskal Wallis: H(2) = 2.76, p = 0.25).
	...
	\caption{\textbf{Number of new logic-77 tasks performed beginning in part 2}. [TODO DESCRIPTION AND STATS]%
	}
	...
	\caption{\textbf{Number of logic-77 tasks performed beginning in part 2}. [TODO DESCRIPTION AND STATS]%
	}
