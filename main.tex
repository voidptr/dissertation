...
\todo[inline]{Add to caption per Charles's comment above}
...
\todo{fix wording above}
%@RCK: Fix awkward sentence above
...
% MJW: As before, I'd change this to something like mutagenic instructions.
affect long-term evolutionary potential. We introduce change-evolved populations from prior experiments with changing environments,
% and the evolution of HGT,
% MJW: For the dissertation, fine; for a manuscript, define HGT when first used.
...
In order to examine the long-term dynamics and mechanisms of evolving populations in changing environments, we performed two sets of experiments. For each set of experiments, we took 
% MJW: Verb tense.  You switch from past to present here.  I have the impression most papers are written as "What we did" in past tense, and "What we find" in present tense, but can't swear to that off the top of my head.  But you switch pretty clearly here to present tense for quite a while.
populations evolved in conditions where short-term evolvability pressures dominated, and we introduced the resulting populations to a completely new environment, with a new set of rewarded bitwise tasks to perform: Logic-77. These new tasks use up to three bitwise inputs rather than a maximum of two, and are each rewarded with a constant 1.2-fold bonus to execution. This reward structure provides a mild selective pressure to evolve these task, but the benefits to performing them do not overwhelm the existing selective pressure to continue performing the tasks rewarded by the original changing or static environment. 
...
The first set of experiments examines the dynamics of populations
% MJW: I'm honestly not sure what this means.  Population dynamics is fairly consistent term in evo., but dynamics of organisms....do you mean dynamics within the organism, like flow control, or the equivalent of metabolism?  More of population dynamics, such as rate of adaptation?  Something else?
%@RCK: Nope, populations :P
that originally evolved in simple changing environments, and are a continuation of the experiments described in Chapter~\ref{chap:ce-adaptation}. 

The second set of experiments evolved population in a more complex changing environment where, rather than evolving to turn a single task on and off, we evolved populations that switch back and forth between performing two sets of task from the Logic-9 environment. The environment and genetic structure of the second set of experiments derives from the experiments described in \ref{chap:evo-hgt}. However, these populations were evolved without a working HGT instruction.
...
\todo{grab sentence below and move to new chapter}
...
% MJW: You're back to past tense here.  I agree with being in past tense here, just flagging the change.
For the first phase (analogous to our earlier experiments), we rewarded a pair of two-input bitwise logical tasks: \texttt{XOR} and \texttt{EQU}. As in our earlier experiments, We rewarded \texttt{XOR} at a constant rate, with an 8-fold bonus to execution, and we alternated rewarding and punishing (or non-rewarding) of the \texttt{EQU} task with a reward that switches between 32-fold bonus to execution and either 0 reward for the benign treatments, or a -32-fold punishment in the harsh treatments. 

%\todo[inline, color=magenta]{CH5 methods - more details about each treatment}
% MJW: Definitely agree.  In particular, I think it's important to explain the terminology of Quiesce.  I was able to infer what it meant from quiescent, but I did still had to look it up.
%@RCK: done  
...
evolved in a more complex cyclic changing environment.  As in the previous chapter, we divided the Logic-9 tasks into two groups (Table~\ref{lte-complex-treatments-logic9}) and alternated reward and punishment between them. For the first phase of our experiments, we alternate rewards between one group and the other, or reward all of Logic-9 for the control treatment. For the benign treatments, rather than punish the alternate group, we simply do not reward the tasks. 
% MJW: You're back to present tense here.
...
In order to compare the effects of different kinds of alternating selective pressures on long-term evolvability, we prepared four different types of experimental treatment using changing environments, plus a static control, for both the simple and complex environmental treatments. 
...
We measure task discovery
% MJW: You're missing the word "discovery" here, right?  Also, parallelism; you capitalized Task Performance, and are not capitalizing task in what I assume is Task Discovery.  I think it's probably better to capitalize neither, but both would be better than just one.
...
We measure task performance by counting the number of unique non-ephemeral tasks that a population performs.  Each task may be performed only once per organism, yielding a maximum task count of 3600 at any given time. Thus, we define a non-ephemeral task as one that a population is performing at equal to or more than 0.1\% level, or 3.6 times per sampling. 
...
that once the Logic-77 reward structure is introduced, populations evolving in harsh changing environments discover many fewer tasks that those evolving in benign changing environments in both the simple environment (Wilcoxon Rank Sum Test: Z = 2.75, p $<$ 0.01) and the complex environment (Wilcoxon Rank Sum Test: Z = 5.96, p $<<$ 0.001) (Figs~\ref{fig:postreward_task_discovery}, \ref{fig:complex-task_discovery}). We hypothesize that this effect is due to the relative differences in the strength of selection between the harsh changing environment and the directional selection toward the logic-77 tasks.
...
Interestingly, in those populations where we removed the harsh environmental change as we introduce Logic-77, we see that task exploration recovers somewhat and reaches a comparable level to the control (Wilcoxon Rank Sum Test: Z = -0.91, p = 0.37). This suggests that it is the active pressures of the harsh environment that depress task exploration, and not a penalty conveyed by architectural features that persist after selective pressures have changed.
...
%=[FIGURE - overall task discovery]
	\begin{figure}[!h]
	\includegraphics[trim={0 0 0 0}, clip, width=0.75\columnwidth]{figures/LTE/lte-simple-overall_task_discovery.png}
	...
	\end{figure}
	\begin{figure}[!h]
	\includegraphics[trim={0 0 0 0}, clip, width=0.75\columnwidth]{figures/LTE/lte-complex-overall_task_discovery.png}
...
\subsubsection{Benign changing environments outperform harsh environments in task performance}
	...
	\todo[inline]{polish figure}
	%=[FIGURE - postreward task performance]
	\begin{figure}[!h]
	\includegraphics[trim={0 0 0 0}, clip, width=0.75\columnwidth]{figures/LTE/lte-simple-task_performance.png}
...
populations seem to outperform the control, this effect is only statistically significant in the complex environment (Wilcoxon Rank Sum Test: Z = 2.42 and 2.37 respectively, p $<$ 0.05). In the simple environment, the differences are not statistically significant (Kruskal Wallis: H(2) = 2.76, p = 0.25).
...
Interestingly, in the complex environments, populations that were initially evolved in a harsh changing environment and then change ceased upon introduction of Logic-77 rewards (Harsh Quiescent)
...
%=[FIGURE - complex environment task discovery]
\todo[inline]{polish figure - fix up figure to include CE uptake value, plus less terrible}
	\begin{figure}[!h]
...
\todo{polish figure - remove non-working Bottlenecks from the figure}
\begin{figure}[h!]
\begin{center}
