...
\documentclass[PhD,final]{msu-thesis}
...
% %Layman's abstract
% \begin{abstract}
% The specific meaning of the term ``evolvability'' is heavily debated, but most definitions can be summarized as: the potential of populations and genomes to produce adaptive variation and complex structures in response to mutation and selection. Changing environments are thought to play a significant role in shaping and promoting evolvability through alternating selective pressures. In this dissertation, I will discuss my recent research on the interplay between changing environments, evolvability, genetic architecture, and the evolution of horizontal gene transfer (HGT), an information-rich mutagenic function that is ubiquitous in nature. I show that changing environments promote short-term evolvability, and move populations to regions of the genotype landscape where new beneficial phenotypes are abundant. I also show that changing environments promote evolution of information-rich mutational operators like horizontal gene transfer. Finally, I show that while changing environments promote long-term evolvability, that other factors such as selection for diversity are also important.
% \end{abstract}
% \clearpage
...
%Evolvability is thought to be created through the interplay of modularity and robustness. Indeed, modularity and robustness build upon each other, layer by layer, to form a framework that produces more powerful adaptive effects while reducing the impact of deleterious mutations.
...
Genetic architecture and population dynamics clearly have a complex interplay in ongoing envolutionary dynamics. Evolutionary history, population diversity, modularity, and task size all play a role in determining the location and characteristics of populations in genotype space, and alter the genotype to phenotype map that permits neutral genetic variation. All of these features contribute to evolvability. In Chapter 2, I demonstrate how changing environments provided a sufficient selective pressure to produce quasi-modular genetic architectures that allow for rapid adaptation to the meta-environment of environmental change.

% \todo{refresh this paragraph to be relevant to the chapter 2}
...
In the fifth chapter, I explore the long-term evolutionary potential of populations evolved in changing environments by evolving two different populations, one evolved in a minimal changing environment, and the other in a rich changing environment, and exposing them to a brand new environment. I demonstrate that while populations adapted to harsh changing environments are indeed able to adapt quickly to previously seen environmental changes, that these populations do not fare as well in brand new environments. Rather, benign changing environments perform best in measures of task discovery and exploration.
...
In the final chapter, I conclude with a synthesis of my results, along with implications for the field, as well as identification of some new directions for pursuing my research into changing environments. 
...
%=4 - some experiments are pretty much impossible.
...
\todo{git repos with data files and ipython notebooks}
...
%All figures are drawn with 95\% confidence intervals, except where explicitly noted otherwise.
...
In order to assess the potential of different regions of the fitness landscape to promote or hinder evolvability, we will use both the \textbf{Genomic Diffusion Rate} ($D_g$) (Eq~\ref{eq:genomic_diffusion_rate}) and the \textbf{Phenotypic Diffusion Rate} ($D_p$) (Eq~\ref{eq:phenotypic_diffusion_rate}), as normalized across changing environments (Eq~\ref{eq:expected_fitness_value}).
...
% \todo[inline, color=yellow]{put in this figure if I have time}
...
% 	\end{figure}
	...
	\caption{\textbf{Genetic architecture of XOR and EQU over time in static environment} for the final dominant genotype in a randomly selected replicate. Proceeding from the left of each figure, each vertical slice represents an organism along the line-of-descent to the final dominant.
...
% @RCK: Performed a correlation between task length and task loss, and find a strong correlation. Because the EQU task is longer is both the changing environments, it'll be lost more easily in both cases.
...
We can predict approximately 47\% of the variation in mean number of second step mutants that regained EQU based on the number of functional and vestigial sites. Most of the variation is predicted by the number of functional sites, though vestigial sites do contribute a small amount. This result is consistent with the role of task length, and thus the number of informational sites, being important for regaining task function. We could not, however, account for all variation, indicating that there are other factors, possibly in robustness or modular architecture of tasks, that are important to this kind evolvability.

	\begin{table}[h]
	\centering
	\caption{\textbf{OLS Regression Results - Mean Fraction of Mutants Regained EQU vs Number of Functional and Vestigial Sites}}
	...
	\bottomrule
	\end{tabular}
	\begin{flushleft}Multiple linear regression, predicting Mean fraction of second-step mutants that regained EQU, based on the number of Functional and Vestigial sites in the original genome.  
	\end{flushleft}
...
%@RCK: added Mike's citations here
...
%@CAO: I think it's fine to have an opening paragraph like this, but I would make it clearer that that's what you're doing. Perhaps just add an aside so that the paragraph begins "Our results, discussed in detail below, show that..."
%@RCK: done
...
%@CAO: I made a few changes (one I commented on) below, but I think it looks good so I commented out the todo.
...
currently intractable.  %@CAO: I think you can make it this direct and simple.
%something that, to a first approximation, we simply can't study in a wet system.  %@CAO: This original form was confusing (talking about wet systems twice?)
...
% MJW: This the sort of thing I was suggesting. I'm slightly brain dead at the moment, but over the next couple of days I'll find a few references for how HGT is essentially ubiquitous and ancient, so in addition to being essentially impossible to test this experimentally in an organic system, we can't even do a comparative study between biological systems than have HGT and equivalent ones which don't.
...
%@CAO: Always be careful when you call something "optimal" since there are many ways of viewing this term. I added "for adaptive evolution".
%@RCK: :thumbsup:
...
%@CAO: This was incorrect. Jeff DOES show that mutation rates go up in a changing environment, they just don't go up much, and nowhere near to the optimal levels for adaptive evolution.
%@RCK: :thumbsup:
...
% @CAO: What do you mean by "fitness effect" here?  Aren't these due to the information in the fragment or the phenotypic changes?  It seems like those would only be helpful if they also increase fitness?  ALSO, make sure you approve of other rewording below.
% @RCK: clarified fixed above. edits look good
...
% @CAO: Added next sentence and adjusted quite a bit of text thereafter.
% @RCK: :thumbsup:
...
%@RCK: fixed
%@CAO: I disagree. Commas are traditionally optional with 4 digits, and only commonly required for 5 or more. That said, I don't have a preference which way to go.
%@RCK: back to 1000 :P
...
%@CAO: It might be worth having a brief sentence or two here, particularly talking about "We conducted a total of 14 (?) treatments in order to isolate the advantages of HGT-Uptake and to isolate the underlying reasons why this instruction is under positive selection. Each treatment differed by either environmental condition (static vs. changing) or the available mutagenic instruction."
...
%@CAO: Did we not also do a pure no-operation version?  That should also be an important baseline.
%@RCK: We did it in chapter 3. If you think it's super important, I'll drag it in.
...
%@CAO: I've split up the above a bit, and added a "when relevant" since some of them (like the mutation event) don't have the recombination site requirements I assume (since I don't see how they would work.)  ALSO, will your readers know what "recombination site requirements" means at this point?  It's possible, I've just lost track.
%@RCK: They all have the same requirements, including ME. Fragments are uptaken, and should you find a homologous match, you apply the mutagen. This makes it so you can actually compare usage rates.
...
%@CAO: Should this next part go above since most of the instructions follow these rules, not just HGT?  Then in this section you can stress that you use the exact, intact, original fragment.
...
%@CAO: I had thought that when you checked your code, it turned out that you always started at the beginning of the genome?
%@RCK: Just went back and checked, and you're right. Corrected
...
%@RCK: moved the above to Ch3 methods
...
%@RCK: fixed
%@CAO: Do you also shuffle the matched sites (that determine the placement)?  I worry that turns out to make this method have a higher effective substitution rate.
%@RCK: Documenting that we looked at it, and there's no substantial difference. Added a todo to myself to put in the figure as a pocket slide for my defense.
    ...
    %@CAO: I added explanation to this effect.
    %@RCK: thx
\end{itemize}	
...
%@RCK: fixed
\\\hhline{|=|=|=|}
HGT & Static & \multirow{2}{*}{ \texttt{HGT} } \\\cline{1-2}
HGT CE & Changing & \\\hline
HGT-Shuffle & Static & \multirow{2}{*}{ \makecell{\texttt{HGT-Shuffle}} } \\\cline{1-2}
HGT-Shuffle CE & Changing & \\\hline
HGT-Random & Static & \multirow{2}{*}{ \makecell{\texttt{HGT-Random}} } \\\cline{1-2}
HGT-Random CE & Changing & \\\hline
ME-Sampled & Static & \multirow{2}{*}{ \makecell{\texttt{ME-Sampled} \\ (Mutation Event - Sampled)} } \\\cline{1-2}
ME-Sampled CE & Changing & \\\hline
ME-Random & Static & \multirow{2}{*}{ \makecell{\texttt{ME-Random} \\ (Mutation Event - Random)} } \\\cline{1-2}
ME-Random CE & Changing & \\\hline
MRI & Static & \multirow{2}{*}{ \makecell{\texttt{MRI} \\ (Mutation Rate Increase)} } \\\cline{1-2}
MRI CE & Changing & \\\hline
Die & Static & \multirow{2}{*}{ \makecell{\texttt{Die}} } \\\cline{1-2}
...
%@RCK: fixed
%@CAO: Changed to "mutagenic instructions".
%@RCK: :thumbsup: looking for this everywhere. :/
...
%@CAO: As I edited the above sentence, I realized that we should have explicitly framed these ideas in term of hypotheses above, not merely questions.
%@RCK: Added an explicit set of hypotheses at the end of the introduction.
...
%This result indicates that not only is information content important, but that localization (clustering) of the mutation effect also matters.

% MJW: Consider expanding on this with speculation as to why it might matter.
%@RCK: speculated :)
%@CAO I would argue that part of the information in a sequence is by necessity encoded in the ordering and relative positioning of instructions. As such, randomizing the positions of those instructions would clearly decrease information content. I think we probably want to frame it that way.
%@RCK: reframed
...
\subsection{Mutation fitness effect correlate with mutagenic instruction use}
	...
	\caption{\textbf{Mutagenic instruction use vs fitness effect of mutation}. The use of the mutagenic instruction correlates with the fitness effect of that instruction (Spearman's Rho: $r_s$ = 0.41,
...
%@RCK: fixed
We measured the proportion of mutations that produce beneficial phenotype changes (Fig~\ref{fig:mutagen_use_vs_fraction_beneficial}). In the context of a changing environment, a mutation that switches an organism's phenotype to match the environment should be strongly selected for. Indeed, we found that this proportion was significantly and substantially higher for HGT mutations (Mdn = 0.041, CI 95\% [0.0291, 0.0463]) than other mutation types (Mdn = 0.004, CI 95\% [0.0036, 0.0053]) (Wilcoxon Rank Sum Test: Z = 8.56, p $<<$ 0.001), and that there was strong correlation between the proportion of beneficial phenotype-switching mutations and use of mutagenic instructions (Spearman's Rho: $r_s$ = 0.59, p =  $<<$ 0.001). This correlation is much stronger than the correlation with fitness effect, suggesting that the magnitude of the fitness effect is less important to selection than the sign of the effect.
...
%* Because HGT is much more beneficial than other types of mutation
%Under Stress, Natural Competence increases
%@CAO: Make sure to change "mutagen" to "mutagenic instructions" (or similar) throughout the next couple of paragraphs.
%@RCK: changed
...
%This confirms that it is not the higher effective mutation rate caused by HGT that is driving its adoption. 
...
One other possibility is that there is a benefit to biasing mutation toward known-useful instructions, such as those collected from living organisms. In this case, we would expect to see a similar increase in use of the \texttt{HGT-Shuffle} instruction. We do observe a substantial increase of this instruction over \texttt{HGT-Random}, however, the increase of \texttt{HGT} is much higher. This is consistent with some benefit occuring from a biased instruction content, but does not account for the entirety of the effect. 
...
%Even so, as we showed in Chapter~\ref{chap:evo-hgt}, HGT use is still modestly elevated even when no new information is provided by a fragment. This suggests that it is not just novel information that conveys a benefit. Rather, it is possible that HGT might be useful for other functions, such as error correction. 

This result is consistent with prior research that shows that populations will tend to depress the use of non-information-rich mutagens, even though, in the long-term, higher mutation rates are likely to be optimal~\cite{clune_natural_2008}. This result is further evidence that selection tends to act in the short term. Thus, even though certain kinds of mutations and architectural features may promote long-term evolvability, these features are hitchhiking on short-term adaptive benefits. Thus, our research shows that even though evolvability may evolve, it is not necessarily selected due to its long-term consequences. 
...
%@RCK: Added an explicit hammering in of the points.
%\todo[color=magenta, inline]{add a takeaway, as per the note above}
...
%In particular, comparing the effects of different kinds of mutations, measuring their use, and isolating their fitness effects are impossible population-level scales. 
Digital evolution allows us to perform the kinds of experiments that otherwise could not be done in natural systems. 
...
% MJW: When my brain is working again, I'll find you a few citations about the ludicrous amount of effort it would take to do the genetic engineering for even just the HGT-Scramble for a handful of HGT events in a biological system. Up to you whether you'd like that for the dissertation, but I think it'll be a good selling point for the paper.
% MJW UPDATE: https://doi.org/10.1093/molbev/msx279 is a recent paper from Tim Coopers group, where the data collection involved making 80 genetic constructs and doing the fitness measurements for each of them. And that is enough for a publication in a good journal last month. 
%@RCK: Integrated reference.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CHAPTER 5 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Methods}
For the experiments described in this chapter, we evolve populations in both minimal and rich environments. Minimal environments have few rewarded tasks, and genomes are fixed length and limited in size. Rich environments allow rewards for many tasks, and genome lengths are allowed to vary.
%used populations of Avida organisms that were originally evolved in other experimental treatments with changing environments.
%@CAO: Rather than the vague sentence above and the more specific below, I would say above "originally evolved in experimental treatments from chapters X and Y." and then change "We used" below just talk about "The" population. HOWEVER it looks like there's only the one previously used population now, so perhaps just merge these sentences to make less awkward?
%@RCK: combined and clarified
For our experiments with minimal environments, we used populations originally evolved in Chapter~\ref{chap:ce-adaptation} to survey the long-term evolutionary potential of populations originating in environments with a single fluctuating task. For the rich environment experiments, we %also  
%We use populations evolved in Chapter~\ref{chap:evo-hgt}, in a more complex changing environment, with the presence of the HGT instruction, evolved both with and without a bonus. And finally, 
...
The goal of our experiments is to evolve populations where short-term evolutionary pressures dominate, and then subject them to an entirely new environment, where we can measure their long-term evolutionary potential. In order to examine the long-term dynamics and mechanisms of evolving populations in changing environments, we performed two sets of experiments, one set with populations evolving in a minimal environment, and the other in a rich environment. 

Each set is composed of five environmental treatments per set, for a total of 10 treatments. Each treatment was divided into two stages, with the first stage focusing on short-term adaptation to a changing environment, and the second stage on long-term evolvability via adaptation to an expanded set of tasks.
%@CAO: I think the experimental design might be clearer if we talk about two "phases" to each treatment rather than "parts".
%@RCK: correcting throughout.

For both set of experiments, we took populations evolved in conditions where short-term evolvability pressures dominated, and introduced them to a completely new environment, with an expanded set of rewarded bitwise tasks to perform: Logic-77. For the purposes of this chapter, we refer to those tasks which were selected for in stage 1 as the \textbf{basic task set}. The Logic-77 task set is a super-set of the basic task set, and includes all bitwise tasks for which there are up to 3 inputs, including those that were initially rewarded in stage 1. We refer to the additional tasks from Logic-77 - those which are not part of the basic tasks set, and that we reward only in stage 2 - as the \textbf{expanded task set}. The total Logic-77 task set is a combination of both the basic and expanded task sets.  
%@CAO: I worry that using "Logic-77" to refer to only the 68 three-input logic tasks might end up pretty confusing to readers (and is different from other papers). Perhaps we should name these the "Basic Environment" for Logic-9 and the "Advanced" environment (or "Complex"?) for Logic-77 *without* Logic-9.
%@RCK: Clarified.
%These new tasks use up to three bitwise inputs rather than a maximum of two in Logic-9.

% MJW: I think you should have a short bit being very explicit about terminology. From reading the rest of this chapter, I have the sense that you are using the "Logic-77 tasks" to refer to those tasks which are rewarded in Logic-77 but are not rewarded in Logic-9. But the tasks that are rewarded in Logic-9 are a strict subset of those rewarded in Logic-77, right?  That is, if an organisms does XOR in Logic-77, it is still rewarded for doing so?  I don't think it's a problem to use Logic-77 tasks to mean this subset of things rewarded in Logic-77, but I think if you do choose to do that, you absolutely need to be explicit upfront that that is how you are using the term.
%@RCK: added
...
The first set of experiments examines the evolutionary dynamics of populations in a \textbf{minimal} changing environments, with a single fluctuating task. These experiments are a continuation of the experiments described in Chapter~\ref{chap:ce-adaptation}. For the second set of experiments, we evolved populations in a \textbf{rich} changing environment where rather than evolving to turn a single task on and off, we evolved populations that switch back and forth between performing two halves of the Logic-9 environment. The environment and genetic structure of the second set of experiments derives from the experiments described in Chapter~\ref{chap:evo-hgt}. However, these populations were evolved without a working HGT instruction.
% MJW: This is more complex than the first set you talk about, but a less complex environment than you're moving them into. I think that can be made clearer.
%@RCK: clarified above
%@CAO: You can have a whole set of new names for the environments that make their role in the experiments clearer...
%@RCK: correcting throughout
The first stage of each experiment is as described above, while the second stage takes the evolved populations and adds the expanded task set. 

%\todo{grab sentence below and move to new chapter or conclusion?}
%For the final set of experiments, we take populations that evolved in a complex changing environment, similar to the second set, but with the addition of the HGT mutagenic instruction. These are a continuation of experiments described in Chapter~\ref{chap:evo-hgt}.

\subsubsection{Environmental Treatments}
As outlined above, for each of our experiment sets,
%@CAO: Why "experiment sets" and not just "experiments"?  And experiment, by its nature, will have multiple treatments...
%@RCK: fixed
...
%@CAO: I don't understand the double use of "treatment" in the sentence beginning above...
%@RCK: clarified. I was differentiating between the quiescent and non-quiescent treatments.
...
%, as we introduce rewards for performing the expanded task set. 
...
As described above, the first set of experiments consisted of four treatments, plus a control, and each treatment was divided into two stages. In the first stage, for a duration of 200,000 updates (analogous to our earlier experiments, detailed in Chapter~\ref{chap:ce-adaptation}), we rewarded a pair of two-input bitwise logical tasks: \texttt{XOR} and \texttt{EQU}. We rewarded \texttt{XOR} at a constant rate, with an 8-fold bonus to execution, and we alternated rewarding and punishing (or non-rewarding) the \texttt{EQU} task with a reward that switches between 32-fold bonus to execution and either 0 reward for the benign treatments, or a -32-fold punishment in the harsh treatment. For the second stage, lasting an additional 200,000 updates, we added a constant reward for executing the expanded task set. Each task in the expanded task set was rewarded with a constant 1.2-fold bonus to execution ($2^{0.3}$). This reward structure provided a mild selective pressure to evolve these task. However, the benefits to performing them do not overwhelm the existing selective pressure to continue performing the basic tasks rewarded by the original changing or static environment. 
	...
	\begin{table}[]
	\centering
	\caption{\textbf{Experimental Treatments - Minimal Cyclic Changing Environment}}
	...
	& & XOR & EQU & XOR & EQU & \makecell{Expanded \\ Task-set \\ (Logic-77 \\ minus \\ XOR \& EQU)} \\\hhline{|=|=|=|=|=|=|=|}
...
% MJW: This is me being overly picky, feel free to ignore. The point mutation probability is 0.00075 per site. But the substitution probability is less than that. Substitutions are defined by changing the sequence. If you call a mutation on a nop-c, there is a non-zero probability of it still being a nop-c. As the number of instructions in the instruction set increases, this probability decreases, but it does mean you have fewer substitutions than you have mutation events.
%Again,
% MJW: I would drop this "again"
%@RCK: removed
...
% MJW: I don't think this should be hyphenated, but am not positive.
%@RCK: leaving it alone
evolved in a richer cyclic changing environment. As in the minimal environment experiment set, we performed four experimental treatments, plus a static control (Table~\ref{cel-treatments-complex}). For the first stage of the experiments, we divided the basic Logic-9 tasks into two groups (Table~\ref{lte-complex-treatments-logic9}) and alternated reward and punishment (or non-reward) between them. For the control treatment, we rewarded all of the Logic-9 basic task set. For the second stage, we introduced rewards for performing the new expanded task set tasks, either with continuing alternating selection (CE), or switching to constant reward (Quiescent). 
	...
	\begin{table}[]
	\centering
	\caption{\textbf{Experimental Treatments - Rich Cyclic Changing Environment}}
	\label{cel-treatments-complex-h}

	\begin{tabular}{|c|c||c||c|c|}
	%\begin{tabular}{ccccccc}
	\hline
	\multirow{3}{*}{ \thead{Treatment} } & \multirow{3}{*}{ \thead{Changing \\ Env. \\ Type} } & \multicolumn{3}{c|}{ \thead{Rewarded Tasks} } \\\cline{3-5}
	& & \multicolumn{1}{c||}{ \thead{Stage 1 \\ (0-200,000 Updates)} } & \multicolumn{2}{c|}{ \thead{Stage 2 \\ (200,000-400,000 Updates)} } \\\cline{3-5}
	& & \makecell{Basic Task-set \\ (Logic-9) \\ Groups \\ A \& B} & \makecell{Basic Task-set \\ (Logic-9) \\ Groups \\ A \& B} & \makecell{Expanded \\ Task-set \\ (Logic-77 \\ minus \\ Logic-9)} \\\hhline{|=|=|=|=|=|}

	Control & \makecell{None \\ (static)} & \makecell{constant  \\ $2^{1-5}$} & \makecell{constant  \\ $2^{1-5}$} & \makecell{constant \\ $2^{0.3}$} \\\hline

	Benign & Cyclic & \makecell{benign \\ fluctuating \\ A: $2^{1-5}$ or 0 \\ B: $2^{1-4}$ or 0} & \makecell{benign \\ fluctuating \\ A: $2^{1-5}$ or 0 \\ B: $2^{1-4}$ or 0} & \makecell{constant \\ $2^{0.3}$} \\\hline
...
% MJW: I know that stats are TODO throughout this chapter, but this sentence in particular absolutely needs them, and it's the sort that would be easy to overlook so I'm flagging it. Though, to be fair, you don't necessarily need the stats if you just lay out the raw numbers -- the selective benefit towards either gaining EQU when it starts being rewarded or lose it when it starts being punished is X, the benefit of acquiring a Logic-77 task is Y.
...
\todo[inline, color=yellow]{IF I HAVE TIME, add result about how minimal also perform more tasks than the rich. this is contrary to our expectation}
...
\todo{look for inconsistent capitalization of treatment names}
...
Our results are consistent across both the minimal and rich environment types. This suggests that regardless of environmental and genomic complexity, changing environments are important drivers of adaptation and long-term evolvability, either directly by driving populations to move across the fitness landscape, or indirectly, by conveying lingering architectural advantages. 
...
% %This result is consistent with prior research on long-term evolvability which identifies population diversity as an important driver of evolvability. common features, such as the gentler strength of sel
...
% % MJW: This is a very short conclusion. I suggest expanding it. In particular, since your results and discussion section doesn't have much about big picture implications of the findings of this chapter, the conclusion section is a good place to expand upon that. If this is a general finding across different systems, what does that mean?
% \todo[inline, color=magenta]{clean up, and fill this conclusion in, recapitulating the argument and framing about balancing of fitness effects, see mike's note. review other conclusion sections for thoughts about how to construct it.}


% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%% Possible Additional Chapter Containing the HGT/LTE stuff
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \chapter{Horizontal Gene Transfer, Changing Environments, and Long Term Evolvability}
% \section{Background}
% \section{Methods}

% \subsubsection{Complex Cyclic Changing Environment with HGT mutagen}
% % MJW: Flagging a mutagen.
...
% % MJW: Still in present tense. Also flagging for the paper that this is referencing a chapter in a way that won't work when they are separate publications, but that doesn't matter for the dissertation.
...
% 	& & XOR & EQU & XOR & EQU & \makecell{Logic-77 } \\\hhline{|=|=|=|=|=|=|=|}
% 	Control & \makecell{None \\ (static)} & \makecell{constant \\ $2^3$} & \makecell{constant \\ $2^5$} & \makecell{constant \\ $2^3$} & \makecell{constant \\ $2^5$} & \makecell{constant \\ $2^{0.3}$} \\\hline
% 	Benign & Cyclic & \makecell{constant \\ $2^3$} & \makecell{benign \\ fluctuating \\ 0 or $2^5$} & \makecell{constant \\ $2^3$} & \makecell{benign \\ fluctuating \\ 0 or $2^5$} & \makecell{constant \\ $2^{0.3}$} \\\hline
...
% % MJW: Made it to this point; need to sleep.
...
% % MJW: Flagging mutagens because I honestly don't know if you mean chemical/physical mutagens in the sense that the term is used in biology/chemistry literature, or the way you've been referring to HGT instructions as mutagens (which I've also been flagging). Until I know which one you mean, I won't be able to suggest citations, because those are different things.
...
% % MJW: This may be an artifact of me editing this over the course of multiple sessions, but do you spell out why we expected this somewhere in this chapter?  If not, I think it would be really helpful. It might even be helpful to give one/ a few sentences here even if you did, as a reminder.
% none of our populations previously evolved to use HGT showed a significant increase in HGT fragment uptakes as a result of being introduced to a new environment\todo{stats}. In general, levels 
% % MJW: Levels of what? Also applies to the next use of levels in this sentence.
...
% % MJW: Have you defined what that means in this chapter?
% results in reduced levels of both task performance and task discovery\todo{stats} as compared to non-Bonus treatments (Fig~\ref{fig:lte-hgt_task_performance}, and Fig~\ref{fig:lte-hgt_task_discovery}).
% \todo{hypothesis here?}
...
% % MJW: I feel like this is awkwardly phrased. I would suggest something more like "One explanation for this result is X. Alternately, this result could be explained by Y."
% More experiments will be required to identify the mechanism behind this effect.
% % MJW: If you are interested in / planning to do these experiments, I suggest identifying what those experiments are / would entail. And if you have sufficient detail to do so, it may be relevant to create a Future Work section/subsection/subsubsection (whichever seems most appropriate)
...
% Again, 
% % MJW: I suggest not beginning sentences with "Again". This can often make it sound like you feel you are belaboring the point, or re-iterating something you've already said.
% consistent with earlier results, populations with HGT follow the same pattern as the earlier experiments. 
% % MJW: Redundant. If this is follows the same patterns as earlier experiments, it is obviously consistent with earlier results.
% Benign-evolved populations strongly outperform Harsh-evolving in measures of Task Discovery and Task Performance\todo{stats} (Fig.~\ref{fig:lte-hgt_task_performance} and Fig.~\ref{fig:lte-hgt_task_discovery}. These results show that regardless of access to an information rich mutagen
% % MJW: Flagging mutagen
% , the dominant effect
% % MJW: of what?
% is one of balancing of fitness pressures. 

% % MJW: General comment: I've just looked through the sections and subsection headings to see if I was right that this could have a somewhat improved narrative flow. I think it could.  
% % One option is to simply move to being more parallel in how things are discussed. For the simple changing environment, task discovery and task performance are different subsections; they're combined in both the complex changing environment and the HGT sections. The task discovery and task performance sections come at the start in the simple and complex changing environments; they come at the end of the HGT section. This jumping around of what is presented when demands a whole lot of attention from the reader.
% % Another option is to make your sections about the metrics, and have subsections about the different environments. So, for example, if you have a section about Task Discovery, and talk about it in each of the three configuration setting pairings you make. I think that would probably be a better narrative structure, because it requires the reader to keep fewer things in memory. When they see that benign changing environments outperform harsh changing environments in task discovery in ALL TYPES OF ENVIRONMENTS/MuTATION SCHEMES YOU LOOK AT in a row, that reduces the memory footprint to "Task discovery is higher for benign changing environments than for harsh changing environments", and then they can go on to consider: What about task performance?

% \section{Conclusion}

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%% END OF Possible Additional Chapter Containing the HGT/LTE stuff
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In this dissertation, we explore the evolutionary dynamics of changing environments, along with their impact on both short and long-term evolvability. We find that changing environments promote short-term adaptation by driving populations to regions of genotype space where more adaptive mutants are available within one or two mutational steps.
%Changing environments accomplish this by creating selective pressures to switch phenotypes, rather than selecting purely for increasing fitness.

Harsh changing environments create selective pressures to switch phenotypes, rather than selecting only for increasing fitness.
These kinds of selective pressures are well suited to promoting the evolution of information-rich mutation types like horizontal gene transfer via transformation. HGT in particular is an excellent adaptation to harsh changing environments, where there is a strong selective pressure for phenotype switching. Indeed, we observe that HGT is much more strongly selected for than other types of mutation operators with similar associated mutation rates.

In the context of long-term evolutionary potential, the benefits of changing environments are less clear. Benign changing environments appear to perform at least, if not more efficiently, than purely directional selection at acquiring traits in a novel environment. While benign changing environments appear to convey some architectural advantages in terms of task discovery and performance, the effects are slight. This result may be due to the types of selection at play in our changing environments. The selective pressures in our changing environments are largely adaptive selective pressures, rather than divergent selective pressures. Divergent selection pressures, such as negative frequency dependence, have been shown to outperform adaptive selection pressures in measures of population-level evolvability~\cite{wilder_reconciling_2015}. Our results are consistent with these findings, and suggest that changing environments alone are insufficient for promoting long-term population-level evolvability. 
...
%For example, the interaction between the HGT grazing bonuses and changing environments is complex, 
%and the interface between adaptation pressures and food rewards i. . 
...
%Overall, we found no significant differences between the number of tasks discovered between treatments with and without a grazing bonus. 
%Further, the task discovery levels were comparable to those in the complex environment. 
	%=[FIGURE - complex environment task discovery]
	\begin{figure}[!h]
	%\missingfigure{HGT Use}
	\includegraphics[trim={0 0 0 0}, clip, width=0.95\columnwidth]{figures/LTE/lte-hgt_bonus_vs_no_bonus_task_discovery.png}
	\caption{\textbf{Task discovery, with and without HGT grazing bonus}. Populations evolving with a grazing bonus had a strong negative correlation between task discovery and HGT use (Spearman's Rho: $r_{s}$ = -0.71, p $<<$ 0.001).  %
	}
	\label{fig:lte-hgt_bonus_vs_no_bonus_task_discovery}
	\end{figure}
Intriguingly, we found a strong negative correlation between the use of a grazing HGT bonus and rates of task discovery (Spearman's Rho: $r_{s}$ = -0.71, p $<<$ 0.001). 
%That is, we found that in replicates where grazing HGT use was higher, task discovery was lower. 
In contrast, we found a much weaker and less robust correlation when no grazing bonus was given for HGT (Spearman's Rho: $r_{s}$ = -0.12, p $<$ 0.01) (Fig~\ref{fig:lte-hgt_bonus_vs_no_bonus_task_discovery}). This result is surprising because we would expect that an information rich mutation operator would promote the discovery of new tasks by combining existing functionality in novel ways. 
%Thus, we would expect that in a context where selection pressures for adaptation are present, that use of HGT would increase. 
