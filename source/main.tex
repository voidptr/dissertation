...
\todo{remove "proposal", revise this paragraph for my thesis statement.}
...
\todo[color=orange]{MORE HERE about other things CE can or can't do}
...
\todo{polish this figure, task layout}
	\begin{figure}[!h]
...
\todo{add figure of the nearby landscape showing how mutations relate 1 and 2 steps out.}
...
\todo{Todo, clarify per Charles's comment above}
...
\todo{TODO - add paragraph describing/speculating about why Benign loses EQU almost as well as Harsh}
...
\todo{add headings below}
\todo{STATS for drop in SCE-Harsh $D_p$}
\todo{reword below}
...
%=HGT umbrella term
Horizontal Gene Transfer (HGT) is an umbrella \todo[noinline]{change to broad?} term for the non-reproductive transfer of genetic material between organisms. Organisms may uptake genes directly from the environment (natural competence\cite{chen_dna_2004}), or else receive them via bacterial conjugation\cite{lederberg_gene_1946} or viral infection (transduction\cite{zinder_genetic_1952,lennox_transduction_1955}). The integrated fragments may either be decomposed inside the recipient cell for their nutrients, or integrated into their genomes. 
...
\todo{More about how this question is still open}
...
\todo{fixed-width font for HGT-Uptake and other Avida instruction names}
...
\section{Results and Discussion}
\todo{fill this in with a general overview of the argument}
...
These results suggests that any direct benefit of using HGT derives from uptaking fragments that match the cycle phase of the environment of the affected organism, and thus likely contains information that relates to that environment. %otherwise, it wouldn't matter where the fragment came from
...
\label{hgt-preferred}
\section{Background}
\todo{add more with some coverage of what research has already been done in this area, and where this all fits in the context}
...
\todo{fill this in with an introduction to the argument we are making}
...
\missingfigure{add the figure for the bottlenecking!!}
...
\todo[inline]{fill this in about the prior research done in this area}


\section{Methods}
\todo[inline]{info about using Avida, but how we used existing code that matches with earlier chapters}
...
\todo[inline]{fill this in, recapitulating the argument and framing}

\chapter{Conclusion}

\todo[inline]{FILL THIS IN - Synthesis of all results and how it all fits together to paint a picture of how CE works.}
